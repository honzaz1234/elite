...
class SeasonUrlDownload():
    ...
    def _page_goalie(self, path, index):
        ...
        subpage_path = path + \
            "?sort-goalie-stats=svp&page-goalie=" + str(index)
        subpage_html = requests.get(subpage_path).content
        selector_subpage = scrapy.Selector(text=subpage_html)
    ...
    def get_player_season_refs(self, league, season):
        ...
        league_stats_path = SeasonUrlDownload.paths["main"] + \
            SeasonUrlDownload.league_refs[league] + \
            SeasonUrlDownload.paths["stats"] + "/" + season
        dict_season = {}
        ...
        number_of_pages_goalies = [re.findall("([0-9]+)#goalies$", string)
                                   for string in ref_last_page if re.search("[0-9]+#goalies$", string)]
        sublist_players = []
    ...
    def get_team_season_refs(self, season, league, ref_list=[]):
        url_season = SeasonUrlDownload.paths["main"] + \
            SeasonUrlDownload.league_refs[league] + "/" + season
