...
PLAYER_URL = "https://www.eliteprospects.com/player/9096/martin-brodeur"
...
ps_o = ps.GoalieScraper(url=PLAYER_URL, page=pl_o.page)
...
input_log_db = ih.LogInputter(
    db_session=session_o.session, 
    scrape_id=1, 
    start_time=start_scrape, 
    end_time=end_scrape, 
    scrape_type="player"
    )

input_db = ih.PlayerHTMLInputter(
    db_session=session_o.session, 
    scraped_data=ps_o.scraped_data, 
    missing_data=ps_o.missing_data, 
    scrape_id=1
    )
input_db.input_data()
input_log_db._input_log()

stg_o = sdg.StorageDBDataGetter(
    db_session=session_o.session, 
    scrape_ids=[1], 
    player_uids=[183442], 
    is_goalie=False
